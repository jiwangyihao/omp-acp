...
| `session/prompt` text/resource_link | 已实现 | baseline text 与 resource_link 可用；实时 Assistant streaming 支持真实 OMP `assistantMessageEvent.text_delta` / `thinking_delta`，并在 `agent_end.messages` 中对未发送的最终 assistant 文本或思考做去重 fallback；收到 `agent_end` 后还会确认 OMP runtime 已 idle，避免 Zed 先显示 turn 结束但下一条 prompt 打到仍 busy 的 OMP；若 client 在普通生成中并发发送下一条 prompt，adapter 会先等待当前活动 prompt cleanup，再把该 ACP request 作为独立的新 OMP `prompt` 发送，避免把两个 ACP turn 合并到同一个响应边界 | `test/unit/translate/prompt.test.ts` + `test/unit/translate/events-message.test.ts` + `test/unit/acp/session-handlers.test.ts` + `test/contract/omp-rpc/rpc-client.test.ts` + `test/smoke/session-prompt.test.ts` |
| `session/cancel` | 已实现 | baseline ACP notification 可用；best-effort 传递 runtime cancel，本地抑制 late chunks；取消后的新 `session/prompt` 会等旧 runtime cleanup 完成后再发往 OMP，避免取消竞态触发 `Agent is already processing` | `test/unit/acp/session-handlers.test.ts` + `test/smoke/session-prompt.test.ts` |
...
| `additionalDirectories`（多根工作区） | 已实现 | 声明 `sessionCapabilities.additionalDirectories:{}`（ACP UNSTABLE 字段）；`session/new`/`load`/`resume`/`fork` 接受 `additionalDirectories`，要求绝对路径（否则 invalid params）、去重并剔除与 `cwd` 相同的条目；额外根目录通过注入的 `additional-directories-extension.mjs` 以 system prompt 段落告知 OMP（`cwd` 仍是唯一工作目录）；OMP session 存储仍按 `cwd` 归档，因此 `session/list` 的 `additionalDirectories` filter 不生效 | `test/unit/session/manager.test.ts` + `test/runtime/omp/command.test.ts` + `test/unit/runtime/omp/additional-directories-extension.test.ts` + `test/unit/acp/initialize.test.ts` |
